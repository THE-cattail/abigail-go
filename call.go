...
func init() {
	bm.AddCommand(&botmaid.Command{
		Do: func(u *botmaid.Update) bool {
			if strings.HasPrefix(u.Message.Text, "咕") {
				if _, ok := callMap[u.Chat.ID]; ok && callMap[u.Chat.ID].Status && callMap[u.Chat.ID].List[botmaid.At(u.User)[0]] {
	...
	bm.AddCommand(&botmaid.Command{
		Do: func(u *botmaid.Update) bool {
			...
			if callMap[u.Chat.ID].List[botmaid.At(u.User)[0]] && !callMap[u.Chat.ID].Resped[botmaid.At(u.User)[0]] {
				callMap[u.Chat.ID].Resped[botmaid.At(u.User)[0]] = true
