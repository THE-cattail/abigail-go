...
type Character struct {
	...
	Description, Thought, Person, Reason, Place, Treasure, Feature string
}
...
var (
	...
	// Feature here.
	Feature = []string{
		"慷慨大方的人。（例如，小费大手，及时雨，慈善家）",
		"善待动物的人。（例如，爱猫人士，农场出生，与马同舞）",
		...
		"忠心在我的人。（例如，背负自己的朋友，从未破誓，为信念而死）",
		"拥有好名头的人。（例如，村里最好的饭后聊天人士，虔信圣徒，不惧任何危险）",
		"雄心壮志的人。（例如，梦想远大，目标是成为 BOSS，渴求一切）",
	}
)

// RollCharacter here.
func RollCharacter() Character {
	c := Character{}
	...
	d1 := random.Int(0, len(Description)-1)
	d2 := 0
	d3 := 0
	for {
		d2 = random.Int(0, len(Description)-1)
		if d2 != d1 {
			break
		}
	}
	for {
		d3 = random.Int(0, len(Description)-1)
		if d3 != d1 && d3 != d2 {
			break
		}
	}
	c.Description = Description[d1] + " " + Description[d2] + " " + Description[d3]
	c.Thought = Thought[random.Int(0, len(Thought)-1)]
	c.Person = Person[random.Int(0, len(Person)-1)]
	c.Reason = Reason[random.Int(0, len(Reason)-1)]
	c.Place = Place[random.Int(0, len(Place)-1)]
	c.Treasure = Treasure[random.Int(0, len(Treasure)-1)]
	c.Feature = Feature[random.Int(0, len(Feature)-1)]
	return c
}
