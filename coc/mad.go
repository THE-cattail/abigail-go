...
func RollMad() string {
	return random.Slice([]string{
		...
		fmt.Sprintf("偏执：你陷入了严重的偏执妄想之中，认为有人在暗中窥视着你，同伴中有人背叛了你，没有人可以信任，万事皆虚，持续 %v 轮。", random.Int(1, 10)),
...
func RollMadSummary() string {
	return random.Slice([]string{
		...
		fmt.Sprintf("暴力倾向：你在 %v 小时后恢复清醒，发现自己似乎曾陷入强烈的暴力与破坏欲之中，但你可能对自己做了什么毫无印象。", random.Int(1, 10)),
		fmt.Sprintf("极端信念：你在 %v 小时后恢复清醒，发现自己曾采取极端和疯狂的表现手段展示你的思想信念。", random.Int(1, 10)),
