...
var (
	// Mad here.
	Mad = []string{
		"失忆：调查员会发现自己只记得最后身处的安全地点，却没有任何来到这里的记忆。这将会持续 %v 轮。",
		"假性残疾：调查员陷入了心理性的失明，失聪以及躯体缺失感中，持续 %v 轮。",
		"暴力倾向：调查员陷入了六亲不认的暴力行为中，对周围的敌人与友方进行着无差别的攻击，持续 %v 轮。",
		"偏执：调查员陷入了严重的偏执妄想之中，持续 %v 轮。有人在暗中窥视着他们，同伴中有人背叛了他们，没有人可以信任，万事皆虚。",
		"人际依赖：守秘人适当参考调查员的背景中重要之人的条目，调查员因为一些原因而将他人误认为了他重要的人并且努力的会与那个人保持那种关系，持续 %v 轮。",
		"昏厥：调查员当场昏倒，并需要 %v 轮才能苏醒。",
		"逃避行为：调查员会用任何的手段试图逃离现在所处的位置，即使这意味着开走唯一一辆交通工具并将其它人抛诸脑后，调查员会试图逃离 %v 轮。",
		"竭嘶底里：调查员表现出%v的极端情绪表现，持续 %v 轮。",
		"恐惧：%v调查员的症状会持续 %v 轮。",
		"躁狂：%v这个症状将会持续 %v 轮。",
	}
	...
	MadFinal = []string{
		...
		"被窃：调查员在 %v 小时后恢复清醒，发觉自己被盗，身体毫发无损。如果调查员携带着宝贵之物（见调查员背景），做幸运检定来决定其是否被盗。所有有价值的东西无需检定自动消失。",
		"遍体鳞伤：调查员在 %v 小时后恢复清醒，发现自己身上满是拳痕和瘀伤。生命值减少到疯狂前的一半，但这不会造成重伤。调查员没有被窃。这种伤害如何持续到现在由守秘人决定。",
		...
		"重要之人：考虑调查员背景中的重要之人，及其重要的原因。在 %v 小时或更久的时间中，调查员将不顾一切地接近那个人，并为他们之间的关系做出行动。",
		"被收容：调查员在精神病院病房或警察局牢房中回过神来，他们可能会慢慢回想起导致自己被关在这里的事情",
		"逃避行为：调查员恢复清醒时发现自己在很远的地方，也许迷失在荒郊野岭，或是在驶向远方的列车或长途汽车上。",
		"恐惧：调查员患上一个新的恐惧症%v。调查员在 %v 小时后回过神来，并开始为避开恐惧源而采取任何措施。",
		"躁狂：调查员患上一个新的躁狂症%v。调查员会在%v小时后恢复理智。在这次疯狂发作中，调查员将完全沉浸于其新的躁狂症状。这症状是否会表现给旁人则取决于守秘人和此调查员。",
	}
	...
	Mania = []string{
		...
		"背德癖（Planomania）：病态地渴求违背社会道德（原文如此，存疑，Planomania 实际上是漂泊症）",
...
func RollMad() string {
	t := random.Int(0, 9)
	if t == 7 {
		return fmt.Sprintf(Mad[t], random.String([]string{
			"大笑",
			"哭泣",
			"嘶吼",
			"害怕",
		...
		return fmt.Sprintf(Mad[t], random.String(Phobia), random.Int(1, 10))
	} else if t == 9 {
		return fmt.Sprintf(Mad[t], random.String(Mania), random.Int(1, 10))
	}
	return fmt.Sprintf(Mad[t], random.Int(1, 10))
}

// RollMadFinal here.
func RollMadFinal() string {
	t := random.Int(0, 9)
	if t == 8 {
		return fmt.Sprintf(MadFinal[t], random.String(Phobia), random.Int(1, 10))
	} else if t == 9 {
		return fmt.Sprintf(MadFinal[t], random.String(Mania), random.Int(1, 10))
	}
	if strings.Contains(MadFinal[t], "%v") {
		return fmt.Sprintf(Mad[t], random.Int(1, 10))
	}
	return MadFinal[t]
}
